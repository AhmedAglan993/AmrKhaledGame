...
public class GameManager : MonoBehaviour
{
	...
	private void Update()
	{
		if (isDissolving)
		{
			foreach (Transform bubble in bubblesToDissolve)
			{
				...
				if (spriteRenderer.material.HasProperty("_DissolveAmount"))
				{
					...
					spriteRenderer.material.SetFloat("_DissolveAmount", dissolveAmount + dissolveSpeed * Time.deltaTime);
				}
				else
				{
					Color c = spriteRenderer.color;
					c.a -= dissolveSpeed * Time.deltaTime;
	...
	public void RestartGame()
	{
		...
		if (GameFlowController.instance != null)
			GameFlowController.instance.ShowMap();
	}
	...
	private void ProcessBottomLimit()
	{
		foreach (Transform t in LevelManager.instance.bubblesArea)
		{
			...
			&& t.position.y < bottomLimit.position.y)
			{
				...
				shootScript.canShoot = false;
				if (GameFlowController.instance != null)
					GameFlowController.instance.ShowLevelLose();
				else
